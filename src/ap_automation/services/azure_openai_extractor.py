...
def _prompt(
    ...
    attachment_summary = [
        {
            ...
            "word_evaluation": (record.get("metadata") or {}).get("word_evaluation") if isinstance(record.get("metadata"), dict) else None,
    ...
    return (
        ...
        "Attachment extractor_selection.selected_extractor identifies the authoritative attachment text source for that attachment: pymupdf means use the top-level attachment text_excerpt from pdf_evaluation, document_intelligence means use successful prebuilt-layout text, word_text means use successful Word text from word_evaluation, and none means no attachment text source was selected. "
        ...
        "These support documents remain AP workflow-relevant even when they are not payable invoices and do not have invoice number or amount fields; do not put invoice-related shift reports, actual-hours reports, timesheets, time sheets, staffing-hours reports, work tickets, labor backup, labor/material breakdowns, or job completion records in excluded_attachments as irrelevant_to_ap_workflow. "
        ...
        "A current internal AP reply answering a prior vendor question with payment timing or status is not itself a vendor question or payment inquiry. "
        ...
        "Classify the item as document_type=\"credit_memo\" when the current email subject, current body, selected attachment text, or document title identifies a vendor credit memo, credit memorandum, credit adjustment, credit note, or issued credit to be handled by AP. "
        "Do not classify an ordinary payable invoice as credit_memo merely because it contains credits/payments rows, prior credits, discounts, negative line adjustments, or payment history. "
        "If the source asks AP to research, explain, match, confirm, or reconcile a credit rather than presenting a credit memo document, classify it as payment_inquiry or vendor_question instead.\n"
        ...
        "Do not set observed_facts.current_invoice_is_past_due=true from quoted history, inherited RE: subject text, or earlier vendor collection language unless the latest body itself asks for or reactivates past-due handling. "
        ...
        "payment_inquiry, credit_memo, past_due_notice, ach_notice, auto_draft_notice, ben_e_keith_notice, lien_release, unknown.\n"
        ...
        "  \"document\": {\n"
        "    \"document_type\": \"invoice|check_request|statement|account_summary|contract|pay_application|vendor_question|payment_inquiry|credit_memo|past_due_notice|ach_notice|auto_draft_notice|ben_e_keith_notice|lien_release|unknown\",\n"
    ...
    "    \"indicates_ach_or_auto_draft\": false,\n"
        ...
        "If flattened PDF text combines an invoice title/address with labels, such as `Invoice 2451 Westlake Parkway INVOICE NO. ACCOUNT NUMBER 231068 5006`, treat `2451 Westlake Parkway` as address evidence and use the value associated with `INVOICE NO.` (`231068`) as invoice.invoice_number; do not use the account number. "
...
def _triage_prompt(parsed_msg: ParsedMsg, attachment_records: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "Invoice-related backup documents such as shift reports, actual hours worked, timesheets, time sheets, staffing-hours reports, work tickets, labor backup, labor/material breakdowns, or job completion records are AP workflow-relevant support and must appear as items when attached with or referenced by an invoice; do not put them in excluded_attachments as irrelevant_to_ap_workflow. "
        "When a payable invoice is paired with that separate support, include separate_supporting_document in the invoice item's risk_flags. "
        ...
        "Do not use the past_due risk_flag from quoted history, inherited reply-chain subject text, or earlier vendor collection language unless email.latest_body_text itself reactivates or asks about past-due, overdue, or collection handling. "
        ...
        "An internal AP reply that answers a prior vendor inquiry by saying payment is scheduled, sent, paid, processed, or will be handled may also be a safe no_detail item when the latest body does not ask for action, cite a current attachment, or introduce a new AP item. "
