...
- Invoice-related backup documents such as shift reports, actual hours worked, timesheets, time sheets, staffing-hours reports, work tickets, labor backup, labor/material breakdowns, and job completion records are AP workflow-relevant supporting documents, even when they are not payable invoices and do not have invoice number or amount fields. They must be returned as items when attached with or referenced by an invoice, not excluded as `irrelevant_to_ap_workflow`.
...
`invoice.invoice_number` must come from a value explicitly labeled as invoice number, invoice no, invoice #, inv no, or equivalent. Extractors must not use service, property, or bill-to address numbers, account numbers, license numbers, dates, amounts, ZIP codes, phone numbers, or line items as invoice numbers. For flattened Forterra-style text such as `Invoice 2451 Westlake Parkway INVOICE NO. ACCOUNT NUMBER 231065 5006`, extract `invoice.invoice_number = "231065"`, treat `5006` as the account number, and preserve `2451 Westlake Parkway` as the service address.
...
Extractors should return `property_lookup` values for database comparison when source text contains property, service, billing, bill-to, shipping, delivery, site, customer/account, facility, work-site, or other property identity signals. These values are normalized structured lookup fields only: `property_code`, `property_name`, `tenant`, `address`, `suite`, `city`, `state`, `zipcode`, and optional `address_candidates`.
...
- Treat labels such as Building, Property, Site, Job, Location, Service Location, Facility, Work Site, Ship To, Deliver To, Sold To, Customer, Account, Attention, and the email subject as property identity sources when they explicitly contain property facts.
- Treat labeled or adjacent `Project`, `Job`, `Site`, `Location`, `Service Location`, `Property`, `Building`, `Facility`, `Work Site`, `Ship To`, `Deliver To`, `Sold To`, `Customer`, `Account`, and `Attention` values as property identity evidence when the visible value contains property, building, site, tenant, address, or asset-code facts. Populate `property_lookup.property_name` with the normalized asset name and `property_lookup.property_code` with the normalized alias/code when the visible value exactly matches or clearly near-matches exactly one configured asset.
- Do not limit property evidence capture to `Bill To` and `Ship To`. When a vendor uses a nonstandard label or an adjacent repeated customer/address block, preserve specific visible asset/property/site names in `property_lookup.property_name` if they match asset reference, or in `business_signals.possible_property_aliases` when uncertain. For example, visible text such as `Hillwood Alliance Group, LP Circle T Golf 2451 Westlake Pkwy` must preserve `Circle T Golf` as property identity evidence.
...
- Do not convert visible Westport/`WP` evidence into Alliance Gateway/`GW` evidence, or visible Gateway/`GW` evidence into Westport/`WP` evidence. Westport and Gateway are distinct asset families even when the building number overlaps. For example, `Service at: WP9 400 Intermodal Pkwy` must not normalize to `GW9` or `Alliance Gateway 9`; it should normalize to `property_lookup.property_code=["wp9"]` and `property_lookup.property_name=["alliance westport 9"]` when supported by `asset_reference`, or omit the property name if not confidently supported. Source text like `Project: 3085 Hillwood Alliance Westport 14 & 15` must not normalize to `GW14`, `GW15`, `Alliance Gateway 14`, or `Alliance Gateway 15`; keep `westport 14 & 15` in `business_signals.possible_property_aliases` and/or `invoice.service_address` unless a configured Westport asset is visibly identified.
...
For thread replies, quoted history, inherited `RE:` subject text, and earlier vendor collection language must not set `observed_facts.current_invoice_is_past_due=true` or triage `past_due` unless `email.latest_body_text` itself reactivates or asks about past-due, overdue, or collection handling. Internal AP payment-status replies such as `scheduled for payment Friday`, `paid via ACH`, `sent for payment`, or `will be processed` are no-action replies when the latest body does not ask for action, cite a current attachment, or introduce a new AP item; they must not set `observed_facts.indicates_vendor_question_or_payment_inquiry=true`.
...
Expected deterministic outcome: `ESCALATE` to the configured `ESCALATE_LINK_ONLY` destination.
...
Expected deterministic outcome: `ESCALATE` to the configured `ESCALATE_MULTI_PDF_MERGE` destination.
...
- Extract only facts explicitly present in the email text, selected attachment text excerpts, or attachment metadata. Selected attachment text excerpts include readable PDF text, Document Intelligence text, and readable Word document text.
...
- Invoice-related shift reports, actual-hours reports, timesheets, time sheets, staffing-hours reports, work tickets, labor backup, labor/material breakdowns, and job completion records remain AP workflow-relevant supporting documents even when they are not payable invoices and do not have invoice number or amount fields; do not put them in `excluded_attachments` as `irrelevant_to_ap_workflow`.
...
Azure OpenAI extraction requests may attach supported original email attachments as additional model context when the configured Azure OpenAI Chat Completions deployment accepts them. Supported attachment context includes PDFs, Word documents, and non-inline images. This attachment context is additive to the existing prompt JSON, selected `text_excerpt`, PyMuPDF metadata, Word text metadata, and Document Intelligence metadata. It must not change the extraction schema, routing hierarchy, deterministic decision authority, or repair/property-review prompt behavior. Audit metadata may record attachment filenames, content types, sizes, and skip reasons, but must not persist base64 attachment payloads in LLM interaction request parameters.

When parsed email HTML is available, Azure OpenAI extraction input may include a sanitized, capped `email.sanitized_body_html` field as additional structure context. Sanitized HTML must be used only to preserve email layout and label/value relationships. It must not be treated as a separate attachment or override `email.latest_body_text` thread-handling rules.
...
- Azure OpenAI extraction and triage requests attach supported original PDF, Word, and non-inline image attachments as additional context when available, without storing base64 payloads in request-parameter audit metadata.
- Azure OpenAI extraction and triage prompts include sanitized email HTML when available as layout context only, without changing email itemization or latest-body thread rules.
...
- Local processor tests prove readable Word attachments are evaluated before LLM triage/detail extraction, exposed as selected `word_text` attachment excerpts, and considered as attachment context like readable PDF excerpts.
...
- Golden tests prove standalone contractor timesheets with no invoice route to `ESCALATE_CONTRACTOR_TIMESHEET`, while invoice plus timesheet backup continues to route through separate-backup escalation to `ESCALATE_MULTI_PDF_MERGE`.
