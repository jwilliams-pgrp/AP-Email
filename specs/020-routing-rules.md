...
- Current replies whose latest body is classified by validated LLM extraction as a no-action acknowledgement, courtesy/social reply, or confirmation that the recipient will handle/process the prior item route to `DISCARD` with destination `NO_ACTION` when deterministic thread and attachment gates pass.
- For current replies, sender domain is not a deterministic no-action eligibility gate. An internal `@hillwood.com` sender is a positive extraction indicator for social/no-action classification only.
- Short replies such as `Thank you. I just sent it.`, `Received, thank you.`, `Great, thanks for the update Leslie!`, or `I resent it.` should set `latest_reply_indicates_no_ap_action=true` only when the latest body does not ask a question, report a wrong destination, cite a current attachment as AP evidence, or introduce new invoice, payment, statement, link, vendor-question, or property-routing facts.
- Internal AP replies that answer a prior vendor inquiry with payment status, such as `scheduled for payment Friday`, `paid via ACH`, `sent for payment`, or `will be processed`, should set `latest_reply_indicates_no_ap_action=true` when the latest body does not ask for action, cite a current attachment, or introduce a new AP item.
