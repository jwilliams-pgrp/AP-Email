...
`db/seed.sql` seeds current local workflow/reference/configuration rows exported from live local Postgres `apautomation` on 2026-06-23. The seeded tables are `routing_destinations`, `ownership`, `asset`, `asset_custom`, `runtime_config`, `no_action_email_patterns`, `workflow_rules`, `workflow_rule_conditions`, and `workflow_rule_versions`. It does not seed operational processing history such as emails, attachments, invoices, extractions, decisions, actions, audit runs, audit steps, LLM interactions, queue rows, or management audit history.

Fresh local and deployment databases include the workflow state currently present in local Postgres, including no-action email patterns, appointment/informational no-action handling, split-multi-PDF handling, matched multifamily invoice routing to `MEDIUS_MF`, zero-dollar invoice escalation to `ESCALATE_0_DOLLAR_INVOICE`, credit memo escalation to `ESCALATE_CREDIT_MEMO`, contractor timesheet escalation, and the current active/disabled destination and rule flags.
