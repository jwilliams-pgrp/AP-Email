...
function New-AzureCompatibleSchemaFile {
    ...
    $temporaryFile = Join-Path ([System.IO.Path]::GetTempPath()) "apautomation-schema-azure-$([System.Guid]::NewGuid()).sql"
...
try {
    ...
    Invoke-PsqlFile -Database $DatabaseName -File $seedFile
